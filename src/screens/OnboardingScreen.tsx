        ...
        <div className="flex flex-col justify-between min-h-screen cyber-bg-dark p-6 text-white transition-colors duration-200 select-none">
            ...
            <div className="flex items-center justify-between pt-2">
                ...
                <button
                    ...
                    className="text-xs font-bold text-cyan-400 hover:text-white px-3 py-1.5 rounded-full glass-panel hover:border-cyan-400 transition-colors"
                >
                    Skip
                </button>
            </div>
            ...
            <div className="flex flex-col items-center text-center my-auto py-8">
                <div className={`w-48 h-48 rounded-full bg-gradient-to-b ${current.bgGradient} flex items-center justify-center text-7xl shadow-[0_0_40px_rgba(0,240,255,0.3)] mb-8 border border-cyan-400/40 animate-scaleIn`}>
                    {current.icon}
                </div>
                ...
                <p className="text-xs text-cyan-200/80 max-w-xs leading-relaxed font-semibold">
                    {current.subtitle}
                </p>
                ...
                <div className="flex items-center gap-2 mt-8">
                    {slides.map((_, idx) => (
                        <div
                            key={idx}
                            className={`h-2.5 rounded-full transition-all duration-300 ${idx === currentSlide ? 'w-8 bg-cyan-400 shadow-[0_0_10px_#00f0ff]' : 'w-2.5 bg-slate-800 border border-cyan-500/20'
                                }`}
                        />
                    ))}
                </div>
